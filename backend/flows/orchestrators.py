        ...
        try:
            ...
            for pdf_file in pdf_files:
                try:
                    ...
                    structured_data = gemini_service.extract_structured_data(extracted_text, pdf_file)
                    ...
                    # Store in Firestore
                    stored_doc_id = firestore_service.store_memo(structured_data, pdf_file)
...
class BenchmarkingFlow:
    ...
    def create_benchmark_analysis(self, memo_text, output_dir="benchmark_output"):
        ...
        try:
            ...
            target_company_data = gemini_service.extract_structured_data(memo_text, "memo_input")
            ...
            competitor_companies = firestore_service.get_sector_competitors(target_company_data)
            ...
            comparison_df = metrics_processor.prepare_comparison_data(target_company_data, competitor_companies)
            ...
            insights = competitive_analyzer.generate_comparison_insights(target_company_data, comparison_df)
            
            # Get sector benchmarks
            benchmarks = competitive_analyzer.get_sector_benchmarks(comparison_df, ' > '.join(sector_hierarchy))
            
            # Generate visualizations
            print("Creating visualizations...")
            chart_paths = self._generate_all_charts(comparison_df, target_company_data, competitor_companies, output_dir)
            ...
            company_name = target_company_data.get('company_overview', {}).get('name', 'target_company')
            safe_company_name = company_name.replace(' ', '_').replace('/', '_').lower()
            pdf_path = os.path.join(output_dir, f"{safe_company_name}_benchmark_report.pdf")
            
            if pdf_report_generator.create_benchmark_report(target_company_data, insights, chart_paths, pdf_path):
    ...
    def _generate_all_charts(self, comparison_df, target_data, competitor_data, output_dir):
        """Generate all visualization charts."""
        chart_paths = {}
        target_company_name = target_data.get('company_overview', {}).get('name', 'Target Company')
        ...
        try:
            if chart_generator.create_radar_chart(comparison_df, target_company_name, radar_path):
        ...
        try:
            if chart_generator.create_bubble_chart(comparison_df, target_company_name, bubble_path):
        ...
        try:
            if chart_generator.create_quadrant_analysis(comparison_df, target_company_name, quadrant_path):
        ...
        try:
            if chart_generator.create_distribution_plots(comparison_df, target_company_name, distribution_path):
        ...
        try:
            benchmarks = competitive_analyzer.get_sector_benchmarks(comparison_df, target_data.get('company_overview', {}).get('sector_hierarchy', [''])[-1])
            if comparison_visualizer.create_scorecard(target_data, benchmarks, scorecard_path):
        ...
        try:
            if comparison_visualizer.create_fundraise_analysis(target_data, competitor_data, fundraise_path):
        ...
        try:
            if comparison_visualizer.create_traction_comparison(target_data, competitor_data, traction_path):
