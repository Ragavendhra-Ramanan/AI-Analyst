...
class ComparisonVisualizer:
    ...
    def create_scorecard(self, target_data, benchmarks, output_path):
        ...
        fig.suptitle(f'{company_name} Performance Scorecard', fontsize=20, fontweight='bold', y=0.95)
        ...
        for i, (label, metric_key) in enumerate(scorecard_metrics):
            ...
            # Target value
            formatted_target = format_currency(target_value) if metric_key in ['revenue', 'cac'] else f"{target_value:.2f}"
            if metric_key == 'gross_margin_pct':
                formatted_target = f"{target_value:.1f}%"
            
            ax.text(0.35, y_pos, formatted_target, fontsize=12, va='center', fontweight='bold')
            
            # Benchmark comparison
            formatted_median = format_currency(median_value) if metric_key in ['revenue', 'cac'] else f"{median_value:.2f}"
            if metric_key == 'gross_margin_pct':
                formatted_median = f"{median_value:.1f}%"
            
            ax.text(0.55, y_pos, f"Median: {formatted_median}", fontsize=11, va='center')
            
            # Performance indicator
            rect = plt.Rectangle((0.75, y_pos - 0.03), 0.2, 0.06, facecolor=color, alpha=0.7, edgecolor='black')
            ax.add_patch(rect)
            ax.text(0.85, y_pos, performance, fontsize=11, va='center', ha='center', fontweight='bold', color='white')
    ...
    def create_fundraise_analysis(self, target_data, competitor_data, output_path):
        ...
        target_fundraise = target_data.get('fundraise') if target_data.get('fundraise') is not None else {}
    ...
    def create_traction_comparison(self, target_data, competitor_data, output_path):
        ...
        target_traction = target_data.get('traction') if target_data.get('traction') is not None else {}
    ...
    def _get_metric_value(self, company_data, metric_key):
        ...
        if metric_key == 'revenue':
            ...
            ltv = financials.get('ltv', {}).get('amount') if financials.get('ltv') else None
    ...
    def _plot_fundraise_timeline(self, ax, fundraise_data):
        ...
        amount_raised_data = fundraise_data.get('amount_raised') if fundraise_data.get('amount_raised') is not None else {}
        amount = amount_raised_data.get('amount', 0) if isinstance(amount_raised_data, dict) else 0
    ...
    def _plot_valuation_comparison(self, ax, target_data, competitor_data):
        ...
        target_valuation = target_data.get('fundraise', {}).get('post_money_valuation', {}).get('amount', 0)
        ...
        for comp in competitor_data[:5]:  # Limit to top 5 competitors
            ...
            comp_val = comp_data.get('fundraise', {}).get('post_money_valuation', {}).get('amount')
            if comp_val:
                valuations.append(comp_val)
                comp_name = comp_data.get('company_overview', {}).get('name', 'Competitor')
