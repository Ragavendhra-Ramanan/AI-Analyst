...
class ChartGenerator:
    ...
    def __init__(self):
        ...
        self.colors = ['#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6', '#1abc9c', '#e67e22']
    ...
    def create_radar_chart(self, metrics_df, target_company_name, output_path):
        ...
        radar_metrics = ['revenue', 'cac', 'ltv_cac_ratio', 'gross_margin_pct', 'valuation']
        available_metrics = []
        ...
        if len(available_metrics) < 3:
            print(f"Not enough metrics available for radar chart (need 3, have {len(available_metrics)})")
            return False
        ...
        for idx, (_, row) in enumerate(normalized_df.iterrows()):
            values = [row[metric] if not np.isnan(row[metric]) else 0 for metric in available_metrics]
            values += values[:1]  # Complete the circle
            
            color = '#e74c3c' if row['company_name'] == target_company_name else self.colors[idx % len(self.colors)]
            linewidth = 3 if row['company_name'] == target_company_name else 2
            alpha = 0.8 if row['company_name'] == target_company_name else 0.6
            
            ax.plot(angles, values, 'o-', linewidth=linewidth, color=color, alpha=alpha, label=row['company_name'])
    ...
    def create_heatmap(self, metrics_df, output_path):
        ...
        numeric_cols = ['revenue', 'cac', 'ltv_cac_ratio', 'gross_margin_pct', 'valuation', 'orders_fulfilled_total']
        ...
        heatmap_data = metrics_df[['company_name'] + available_cols].set_index('company_name')
        ...
        heatmap_data = heatmap_data.dropna(how='all', axis=1)  # Drop columns with all NaN
    ...
    def create_bubble_chart(self, metrics_df, target_company_name, output_path):
        ...
        if metrics_df.empty or 'revenue' not in metrics_df.columns or 'valuation' not in metrics_df.columns:
            return False
        ...
        for _, row in bubble_data.iterrows():
            color = '#e74c3c' if row['company_name'] == target_company_name else '#3498db'
            size = 1000 if row['company_name'] == target_company_name else 500
            alpha = 0.8 if row['company_name'] == target_company_name else 0.6
            
            plt.scatter(row['revenue'], row['valuation'], s=size, alpha=alpha, color=color, edgecolors='black', linewidth=2)
            plt.annotate(row['company_name'], (row['revenue'], row['valuation']), 
    ...
    def create_quadrant_analysis(self, metrics_df, target_company_name, output_path):
        ...
        if metrics_df.empty or 'revenue' not in metrics_df.columns or 'ltv_cac_ratio' not in metrics_df.columns:
            return False
        ...
        for _, row in quadrant_data.iterrows():
            ...
            ax.scatter(row['revenue'], row['ltv_cac_ratio'], s=size, alpha=alpha, color=color, edgecolors='black', linewidth=2)
            ax.annotate(row['company_name'], (row['revenue'], row['ltv_cac_ratio']), 
                       xytext=(5, 5), textcoords='offset points', fontsize=10, fontweight='bold')
        
        # Add quadrant labels
        ax.text(0.25, 0.95, 'High Efficiency\nLow Revenue', transform=ax.transAxes, ha='center', va='top', 
               bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7), fontweight='bold')
        ax.text(0.75, 0.95, 'High Efficiency\nHigh Revenue', transform=ax.transAxes, ha='center', va='top', 
               bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.7), fontweight='bold')
        ax.text(0.25, 0.05, 'Low Efficiency\nLow Revenue', transform=ax.transAxes, ha='center', va='bottom', 
               bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.7), fontweight='bold')
        ax.text(0.75, 0.05, 'Low Efficiency\nHigh Revenue', transform=ax.transAxes, ha='center', va='bottom', 
        ...
        ax.set_title('Revenue vs LTV/CAC Quadrant Analysis', fontsize=16, fontweight='bold')
    ...
    def create_distribution_plots(self, metrics_df, target_company_name, output_path):
        ...
        available_metrics = [m for m in distribution_metrics if m in metrics_df.columns and not metrics_df[m].isna().all()]
        ...
        for idx, metric in enumerate(available_metrics):
            ...
            ax.hist(data, bins=min(10, len(data)), alpha=0.7, color='lightblue', edgecolor='black')
            
            # Mark target company
            target_value = metrics_df[metrics_df['company_name'] == target_company_name][metric].iloc[0] if not metrics_df[metrics_df['company_name'] == target_company_name].empty else None
            if target_value and not np.isnan(target_value):
                ax.axvline(x=target_value, color='red', linestyle='--', linewidth=2, label=f'{target_company_name}')
                ax.legend()
