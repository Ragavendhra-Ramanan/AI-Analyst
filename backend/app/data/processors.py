...
class MetricsProcessor:
    ...
    def extract_metrics_for_comparison(self, companies_data):
        ...
        for company in companies_data:
            try:
                ...
                company_name = company_data.get("company_overview", {}).get(
                    "name", company["id"]
                )
                ...
                metrics = {
                    ...
                    "sector": " > ".join(
                        company_data.get("company_overview", {}).get(
                            "sector_hierarchy", ["Unknown"]
                        )
                    ),
                    "stage": company_data.get("company_overview", {}).get(
                        "stage", "Unknown"
                    ),
                    "founding_year": company_data.get("company_overview", {}).get(
                        "founding_year"
                    ),
                    # Financial metrics
                    "revenue": safe_extract_amount(
                        company_data.get("financials", {}).get("revenue")
                    ),
                    "gmv": safe_extract_amount(
                        company_data.get("financials", {}).get("gmv")
                    ),
                    "cac": safe_extract_amount(
                        company_data.get("financials", {}).get("cac")
                    ),
                    "aov": safe_extract_amount(
                        company_data.get("financials", {}).get("aov")
                    ),
                    "gross_margin_pct": company_data.get("financials", {}).get(
                        "gross_margin_pct"
                    ),
                    # Market metrics
                    "tam": safe_extract_amount(
                        company_data.get("market", {}).get("tam")
                    ),
                    "sam": safe_extract_amount(
                        company_data.get("market", {}).get("sam")
                    ),
                    # Traction metrics
                    "orders_fulfilled_total": company_data.get("traction", {}).get(
                        "orders_fulfilled_total"
                    ),
                    "repeat_rate_pct": company_data.get("traction", {}).get(
                        "repeat_rate_pct"
                    ),
                    # Fundraise metrics
                    "valuation": self._extract_valuation(
                ...
                print(
                    f"Error extracting metrics for company {company.get('id', 'Unknown')}: {e}"
                )
                continue
    ...
    def prepare_comparison_data(self, target_company_data, competitor_companies):
        ...
        if target_name and not any(
            comp["data"].get("company_overview", {}).get("name") == target_name
            for comp in all_companies
        ):
            all_companies.append({"id": "target_company", "data": target_company_data})
