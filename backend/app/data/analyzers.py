...
class CompetitiveAnalyzer:
    ...
    def generate_comparison_insights(
        self, target_data: Dict[str, Any], df
    ) -> Dict[str, Any]:
        ...
        if target_metrics["revenue"] and not competitor_data["revenue"].isna().all():
            revenue_percentile = self._calculate_percentile(
                target_metrics["revenue"], competitor_data["revenue"]
            )
            insights.append(
                f"Revenue performance: {revenue_percentile:.0f}th percentile among peers"
            )
            ...
            if revenue_percentile < 25:
                ...
                recommendations.append(
                    "Strong revenue position - consider scaling strategies"
                )

        # CAC comparison
        if target_metrics["cac"] and not competitor_data["cac"].isna().all():
            cac_percentile = 100 - self._calculate_percentile(
                target_metrics["cac"], competitor_data["cac"]
            )
            insights.append(
                f"Customer Acquisition Cost: {cac_percentile:.0f}th percentile (lower is better)"
            )

            if cac_percentile < 25:
                recommendations.append(
                    "High CAC - optimize marketing channels and conversion"
                )

        # LTV/CAC ratio comparison
        if (
            target_metrics["ltv_cac_ratio"]
            and not competitor_data["ltv_cac_ratio"].isna().all()
        ):
            ltv_cac_percentile = self._calculate_percentile(
                target_metrics["ltv_cac_ratio"], competitor_data["ltv_cac_ratio"]
            )
            insights.append(
                f"LTV/CAC ratio: {ltv_cac_percentile:.0f}th percentile among peers"
            )

            if ltv_cac_percentile < 25:
                recommendations.append(
                    "Improve customer lifetime value or reduce acquisition costs"
                )

        # Gross margin comparison
        if (
            target_metrics["gross_margin_pct"]
            and not competitor_data["gross_margin_pct"].isna().all()
        ):
            margin_percentile = self._calculate_percentile(
                target_metrics["gross_margin_pct"], competitor_data["gross_margin_pct"]
            )
            insights.append(
                f"Gross margin: {margin_percentile:.0f}th percentile among peers"
            )

            if margin_percentile < 25:
                recommendations.append(
                    "Focus on improving unit economics and cost structure"
                )

        # Valuation comparison
        if (
            target_metrics["valuation"]
            and not competitor_data["valuation"].isna().all()
        ):
            valuation_percentile = self._calculate_percentile(
                target_metrics["valuation"], competitor_data["valuation"]
            )
            insights.append(
                f"Valuation: {valuation_percentile:.0f}th percentile among peers"
            )
        ...
        return {
            ...
            "competitive_position": self._assess_competitive_position(
                target_metrics, competitor_data
            ),
        }
        ...
        for metric in key_metrics:
            if target_metrics[metric] and not competitor_data[metric].isna().all():
                percentile = self._calculate_percentile(
                    target_metrics[metric], competitor_data[metric]
                )
    ...
    def get_sector_benchmarks(self, df, target_sector: str) -> Dict[str, Any]:
        ...
        sector_data = df[
            df["sector"].str.contains(
                target_sector.split(" > ")[-1], case=False, na=False
            )
        ]
