...
class GeminiService:
        ...
        try:
            ...
            if response and response.text:
                ...
                structured_data["metadata"] = {
                    ...
                    "extraction_date": json.loads(
                        json.dumps(structured_data.get("metadata", {}))
                    ).get("extraction_date", "2025-09-20"),
    ...
    def generate_competitive_summary(
        self, target_data, competitor_data, insights, benchmarks
    ):
        ...
        try:
            ...
            summary_prompt = self._create_competitive_summary_prompt(
                target_data, competitor_data, insights, benchmarks
            )
    ...
    def _create_competitive_summary_prompt(
        self, target_data, competitor_data, insights, benchmarks
    ):
        ...
        sector = " > ".join(
            target_data.get("company_overview", {}).get(
                "sector_hierarchy", ["Unknown Sector"]
            )
        )
        ...
        revenue = (
            financials.get("revenue", {}).get("amount")
        ...
        valuation = (
            fundraise.get("post_money_valuation", {}).get("amount")
            if fundraise.get("post_money_valuation")
            else None
        )
        ...
        competitor_names = [
            comp.get("data", {}).get("company_overview", {}).get("name", "Unknown")
